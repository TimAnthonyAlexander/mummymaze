...
This is an original, from-scratch reimplementation of the *genre's* mechanics for personal, offline play. Code and assets are original or licensed for this use: the character sprites are hand-drawn SVG, the sound effects are short recorded samples (mostly public-domain CC0; full provenance and licences in `src/assets/audio/CREDITS.md`), and every level layout is generated or hand-authored here. Game rules and mechanics are not copyrightable; only specific expression (art, audio, level data, names, logos) is. This project does not use, and is not affiliated with, any specific commercial game's name, branding, artwork, audio, or level data.
...
- **Sound with toggles.** Short recorded sound effects (footsteps, latches, impacts, chimes, plus a beast roar and a stone-drag for the spawn intro), with mute and animation toggles. Everything degrades to silence and never throws when Web Audio is unavailable; provenance and licences are tracked in `src/assets/audio/CREDITS.md`.
